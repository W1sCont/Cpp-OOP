...
template <typename T>
List<T>::List(const List<T>& L) : Head{ nullptr }, Tail{ nullptr }, Count{ 0 }
{
	Elem<T>* temp = L.Head;
...
template <typename T>
bool List<T>::operator!=(const List<T>& L) { return !(*this == L); }
...
int main()
{
	List<int> l1{};
	int n{1};
	cout << "Enter number or 0 for end:\n";
	while (n) {
		cout << "> ";
		cin >> n;
		if (cin.fail()) {
			cin.clear();
			cin.ignore(1000, '\n');
			cout << "Its not a number, default 1\n";
			n = 1;
		}
		if (n == 0) break;
		l1.AddHead(n);
